...
DR_PMTSD::ProcessHits_constStep( const G4Step* aStep, G4TouchableHistory* )
{
  ...
  if (true /*processName == "Cerenkov"*/)
  {
    ...
    if (thePostPVNamesd.contains("ecalDetP_ff"))
    {
      ...
      if (processName == "Cerenkov"){ CreateTree::Instance()->h_phot_cer_lambda_ECAL_f_Ceren->Fill(photWL);CreateTree::Instance()->SDdetected_ff_C++;}
      if (processName == "Scintillation"){ CreateTree::Instance()->h_phot_cer_lambda_ECAL_f_Scin->Fill(photWL);CreateTree::Instance()->SDdetected_ff_S++;}
    }
    ...
    else if (thePostPVNamesd.contains("ecalDetP_rr"))
    {
      ...
      hit->SetBounceCount(theTrack->GetCurrentStepNumber());
      fPMTHitsCollection->insert(hit);
      if (processName == "Cerenkov"){ CreateTree::Instance()->h_phot_cer_lambda_ECAL_r_Ceren->Fill(photWL);      CreateTree::Instance()->SDdetected_rr_C++;}
      if (processName == "Scintillation"){ CreateTree::Instance()->h_phot_cer_lambda_ECAL_r_Scin->Fill(photWL);      CreateTree::Instance()->SDdetected_rr_S++;}     
    }else{
